...
const HomeBanners = ({ getDeleted }) => {
  ...
  const [selectedHomeBanner, setSelectedHomeBanner] = useState([]);
  ...
  const openDeleteModal = (homeBannerId) => { setHomeBannerToDelete(homeBannerId); setIsBulkDelete(false); setShowDeleteModal(true); };
  const openBulkDeleteModal = () => { setHomeBannerToDelete(null); setIsBulkDelete(true); setShowDeleteModal(true); };
  const closeDeleteModal = () => { setHomeBannerToDelete(null); setShowDeleteModal(false); };

  const handleDeleteConfirm = async () => {
    if (isBulkDelete) {
      try {
        const res = await axios.delete(`${API_BASE_URL}/home_banners/delete-selected`, {
          data: { ids: selectedHomeBanner }
        });
        setData((prevData) => prevData.filter((item) => !selectedHomeBanner.includes(item.id)));
        setTotalRecords((prev) => prev - selectedHomeBanner.length);
        setFilteredRecords((prev) => prev - selectedHomeBanner.length);
        setSelectedHomeBanner([]);
        showNotification(res.data?.message || "Selected home banner deleted successfully!", "success");
      } catch (error) {
        console.error("Error deleting selected home banner:", error);
        showNotification("Failed to delete selected home banner.", "error");
      } finally {
        closeDeleteModal();
      }
    } else {
  ...
  const handleSelectAll = (event) => {
    if (event.target.checked) {
      setSelectedHomeBanner(data?.map((item) => item.id));
    } else {
      setSelectedHomeBanner([]);
    }
  };

  const handleSelectHomeBanner = (homeBannerId) => {
    setSelectedHomeBanner((prevSelectedHomeBanner) =>
      prevSelectedHomeBanner.includes(homeBannerId)
        ? prevSelectedHomeBanner.filter((id) => id !== homeBannerId)
        : [...prevSelectedHomeBanner, homeBannerId]
    );
  };
    ...
    <>
      <div className="page-wrapper">
        <div className="page-content">
          <Breadcrumb page="Settings" title={getDeleted ? "Recently Deleted Home Banner" : "Home Banner"}
          add_button={!getDeleted && (<><i className="bx bxs-plus-square"></i> Add Home Banner</>)} add_link="#" onClick={() => openForm()}
          actions={
            <>
          ...
          <div className="row">
            ...
            <div className={!getDeleted ? "col-md-7" : "col-md-12"}>
              <div className="card">
                <div className="card-body">
                  <h5 className="card-title mb-3">{!getDeleted ? "Home Banner List" : "Recently Deleted Home Banner List"}</h5>
                  {getDeleted && (
