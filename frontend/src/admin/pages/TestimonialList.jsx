...
const TestimonialList = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${API_BASE_URL}/testimonials/server-side`, {
  ...
  const openModal = async (editData = null) => {
    ...
    setErrors({});
    if (editData) {
      const res = await axios.get(`${API_BASE_URL}/testimonials/${editData.id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await axios.put(`${API_BASE_URL}/testimonials/${formData.id}`, payload, {
        ...
        const img = await axios.get(`${API_BASE_URL}/files/${res.data.testimonials.file_id}`);
        const updatedFileName = img.data.file;
        const payload1 = { ...res.data.testimonials, file_name: updatedFileName };
  ...
  const openDeleteModal = (testimonialId) => { setTestimonialToDelete(testimonialId); setShowDeleteModal(true); };

  const closeDeleteModal = () => { setTestimonialToDelete(null); setShowDeleteModal(false); };

  const handleDeleteConfirm = async () => {
    try {
      await axios.delete(`${API_BASE_URL}/testimonials/${testimonialToDelete}`);
      setData((prevData) => prevData.filter((item) => item.id !== testimonialToDelete));
  ...
  const handleStatusConfirm = async () => {
    ...
    try {
      await axios.patch(`${API_BASE_URL}/testimonials/${id}/status`, { status: newStatus });
    ...
    <>
      <div className="page-wrapper">
        <div className="page-content">
          <Breadcrumb page="Settings" title="Testimonial" add_button="Add Testimonial" add_link="#" onClick={openAddModal} />
          <div className="card">
