...
const ProductsList = () => {
  ...
  const [itemSubCategorySearchTerm, setItemSubCategorySearchTerm] =
    useState("");
      ...
      (async () => {
        try {
          const res = await axios.get(
            `${API_BASE_URL}/products/item-hierarchy/${itemId}`
          );
          const data = res.data;
          ...
          setSelectedItemCategories(
            data.item_category_id ? [data.item_category_id] : []
          );
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        ...
        const res = await axios.post(
          `${API_BASE_URL}/items/by-selected-category-subcategory-itemcategory-itemsubcategory`,
          {
            categories: selectedCategories,
            subcategories: selectedSubCategories,
            itemCategories: selectedItemCategories,
    ...
    return (
      <>
        {items.map((_, i) => (
          <div
            key={i}
            className={isListView ? "col-md-6 mb-4" : "col-lg-4 col-sm-6 mb-4"}
          >
            <div
              ...
              <div className="d-lg-flex d-none gap-2 align-items-center">
                <button
                  className={`btn btn-sm text-nowrap ${
                    !isListView ? "btn-orange" : "btn-outline-white text-white"
                  }`}
              ...
              <div className="d-sm-flex align-items-baseline justify-content-between gap-2 mb-2">
                <div className="d-flex align-items-center gap-2 flex-wrap">
                  ...
                  {selectedItems.map((id) => (
                    <span
                      key={`itm-${id}`}
                      className="badge bg-dark text-white d-flex align-items-center"
                    >
                  ...
                  {selectedStates.map((id) => (
                    <span
                      key={`state-${id}`}
                      className="badge bg-success text-white d-flex align-items-center"
                    >
              ...
              {loading ? (
                ...
                filteredProducts.map((product) => (
                  <div
                    key={product.id}
                    className={
                      isListView ? "col-md-6 mb-4" : "col-lg-4 col-sm-6 mb-4"
                    }
                  >
                    <div
                      className={`card text-dark border overflow-hidden products-list-cards ${
                        isListView ? "flex-row" : "h-100"
                      }`}
                      style={{ height: isListView ? 200 : "auto" }}
                    >
